...
class _MyUpcomingFittingsListState extends State<MyUpcomingFittingsList> {
  ...
  void loadMyUpcomingFittingsList() {
    ...
    List<FittingRenter> allFittingRenters = List.from(
        Provider.of<ItemStoreProvider>(context, listen: false).fittingRenters);
    ...
    for (FittingRenter dr in allFittingRenters) {
      DateTime convertedDate =
          DateFormat('yyyy-MM-ddTHH:mm:ss').parse(dr.bookingDate);
    ...
    return Consumer<ItemStoreProvider>(builder: (context, value, child) {
      return ListView.builder(
          ...
          itemBuilder: (BuildContext context, int index) {
            return (upcomingFittingsList.isNotEmpty)
                ? MyUpcomingFittingsImageWidget(
                    upcomingFittingsList[index],
                    upcomingFittingsList[index].itemArray,
                    upcomingFittingsList[index].bookingDate,
                    upcomingFittingsList[index].price,
                    upcomingFittingsList[index].status)
